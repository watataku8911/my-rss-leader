...
export default function Home() {
  ...
  const getCookpad = async (): Promise<Items[]> => {
    const endpoint: string = "https://api.rss2json.com/v1/api.json";
    const feed_url: string = "https://techlife.cookpad.com/feed";
  ...
  useEffect(() => {
    const cookpadFeed: Promise<Items[]> = getCookpad();
    cookpadFeed.then((datalist) => {
      setLists(datalist);
    });
  }, []);
  ...
  return (
    <div className={styles.container}>
      <Head>
        <title>my-rss-leader｜Coocpad開発者ブログ</title>
        <meta
          name="description"
          content="Coocpad開発者ブログを取得してみました。"
        />
